...
class CompressingBERT(MovingCameraScene):
    def construct(self):
        ...
        for i, colour in enumerate(colour_list, start=1):
            input_horizontal = Text("Input: ", color=WHITE).scale(0.4).shift(0.8 * UP + 3.1 * LEFT + shift_count * RIGHT)
            calc_1_horizontal = Text("Calc 1: ", color=WHITE).scale(0.4).shift(0.8 * UP + 1.6 * LEFT + shift_count * RIGHT)
            calc_2_horizontal = Text("Calc 2: ", color=WHITE).scale(0.4).shift(0.8 * DOWN + 1.6 * LEFT + shift_count * RIGHT)
            calc_3_horizontal = Text("Calc 3: ", color=WHITE).scale(0.4).shift(0.8 * UP + 0.7 * RIGHT + shift_count * RIGHT)

            arrow_1_horizontal = Arrow(buff=0.05, start=0.35 * UP + 3.65 * LEFT, end=0.35 * UP + 2.3 * LEFT,
                                        stroke_width=2, max_tip_length_to_length_ratio=0.12).shift(shift_count * RIGHT)
            arrow_2_horizontal = Arrow(buff=0.05, start=0.6 * UP + 1.25 * LEFT, end=0.6 * DOWN + 1.25 * LEFT,
                                        stroke_width=2, max_tip_length_to_length_ratio=0.13).shift(shift_count * RIGHT)
            arrow_3_horizontal = Arrow(buff=0.05, start=0.85 * DOWN + 0.2 * RIGHT, end=0.6 * UP + 1.05 * RIGHT,
                                        stroke_width=2, max_tip_length_to_length_ratio=0.085).shift(shift_count * RIGHT)

            weight_1_value = weight_1.pop().shift(0.6 * LEFT + 0.77 * UP + shift_count * RIGHT)
            weight_2_value = weight_2.pop().shift(0.54 * LEFT + 0.81 * DOWN + shift_count * RIGHT)
            weight_3_value = weight_3.pop().shift(1.6 * RIGHT + 0.8 * UP + shift_count * RIGHT)

            input_value = inputs_outputs.pop().shift(2.5 * LEFT + 0.8 * UP + shift_count * RIGHT)

            box = SurroundingRectangle(point_t, point_u, color=WHITE, stroke_width=2).shift(shift_count * RIGHT)

            # --- Store values --- #
            box_dic[f"box_{i}"] = VGroup(box)
            text_dic[f"text_{i}"] = VGroup(arrow_1_horizontal, arrow_2_horizontal, arrow_3_horizontal,
                                        calc_1_horizontal, calc_2_horizontal, calc_3_horizontal,
                                        input_horizontal, input_value,
                                        weight_1_value, weight_2_value, weight_3_value)
            weight_dic[f"weight_{i}"] = VGroup(weight_1_value, weight_2_value, weight_3_value)
            input_dic[f"input_{i}"] = VGroup(input_value)
            horizontal_vgroup.add(box, arrow_1_horizontal, arrow_2_horizontal, arrow_3_horizontal,
                                calc_1_horizontal, calc_2_horizontal, calc_3_horizontal,
                                input_horizontal, weight_1_value, weight_2_value, weight_3_value, input_value)

            # Animate originals weights
            self.play(AnimationGroup(
                Create(arrow_1_horizontal), Create(input_horizontal), FadeIn(input_value),
                Create(calc_1_horizontal), FadeIn(weight_1_value),
                Create(arrow_2_horizontal), Create(calc_2_horizontal), FadeIn(weight_2_value),
                Create(arrow_3_horizontal), Create(calc_3_horizontal), FadeIn(weight_3_value),
                Create(box),
                lag_ratio=0.5, run_time=5
            ))

            # --- Add coloured weights --- #
            weight_1_value_colour = colour[2].copy().shift(0.6 * LEFT + 0.77 * UP + shift_count * RIGHT)
            weight_2_value_colour = colour[1].copy().shift(0.54 * LEFT + 0.81 * DOWN + shift_count * RIGHT)
            weight_3_value_colour = colour[0].copy().shift(1.6 * RIGHT + 0.8 * UP + shift_count * RIGHT)
            ...
            # Shifting the camera and boxes to the right by 6 units
            shift_count += 6
            camera_shift += 6
            
            # Adding the coloured weights to the animation
            self.play(AnimationGroup(
                FadeIn(weight_1_value_colour, weight_2_value_colour, weight_3_value_colour),
        ...
        # Adding the output text and arrow
        ...
        # Moving the camera back to the origin
        self.play(self.camera.frame.animate.move_to(ORIGIN), run_time= 5, rate_func= smoothstep)
        ...
        self.play(AnimationGroup(self.camera.frame.animate.move_to(box_dic["box_3"]).set(width= horizontal_vgroup.width + 2), rate_func= linear, run_time= 5))

        self.wait(1)

        # Starting the compression animation
        ...
        lpddr_group = VGroup(lpddr_1_1, lpddr_1_2, lpddr_1_3, lpddr_2_1, lpddr_2_2, lpddr_2_3,
                            lpddr_3_1, lpddr_3_2, lpddr_3_3, lpddr_4_1, lpddr_4_2, lpddr_4_3,
                            lpddr_5_1, lpddr_5_2, lpddr_5_3, lpddr_text, lpddr_box)
        ...
        # --- Adding Background Image Fade-In --- #
        background = ImageMobject("speedAI240_lowres_bkg_light (1).jpg")  # Image path
        background.scale_to_fit_height(8)
        background.set_z_index(-10)  # Send image to the back all the elements
        background.scale(5)
        ...
        self.play(AnimationGroup(
                  ...
                  *[FadeOut(mob)for mob in self.mobjects],
                  self.camera.frame.animate.scale(4.7),  # Adjust scale to zoom out
                  background.animate.set_opacity(1),  # Fade in the background image
                  run_time= 5,
                  ))